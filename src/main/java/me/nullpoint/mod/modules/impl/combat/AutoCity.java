...
public class AutoCity extends Module {
	...
	public final SliderSetting targetRange =
			add(new SliderSetting("TargetRange", 6.0, 0.0, 8.0, 0.1));
	public AutoCity() {
	...
	private void doBreak(PlayerEntity player) {
				...
				for (double y : yOffset) {
					for (double x : xzOffset) {
						for (double z : xzOffset) {
							...
							if (isObsidian(offsetPos) && offsetPos.equals(SpeedMine.breakPos)) {
								return;
							}
						}
					}
		...
		if (surround.getValue()) {
			...
			for (Direction i : Direction.values()) {
				if (i == Direction.UP || i == Direction.DOWN) continue;
				if (!SpeedMine.godBlocks.contains(getBlock(pos.offset(i))) && !(getBlock(pos.offset(i)) instanceof BedBlock)) {
	...
	private boolean isObsidian(BlockPos pos) {
		return (getBlock(pos) == Blocks.OBSIDIAN || getBlock(pos) == Blocks.ENDER_CHEST) && BlockUtil.getClickSideStrict(pos) != null && (!pos.equals(SpeedMine.secondPos) || !(mc.player.getMainHandStack().getItem() instanceof PickaxeItem || SilentDouble.INSTANCE.isOn()));
	}
}